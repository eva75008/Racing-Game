...
#https://www.mecanicascience.fr/article/la_3d_par_la_perspective&articleview&7#iiducode
...
class Milestone:
    ...
    def distance(self):
        return sqrt((screen_width // 2 - self.x)**2 +
        (screen_height // 2 - self.y)**2)

    def right(self):
        self.x = self.x * 1.2
        y = (self.max_width * distance()) // sqrt((screen_width//2)**2 + (screen_height //2)**2)
        pyxel.rect(
                self.x,
                self.y,
